...
public class JdbcConvention extends Convention.Impl {
  ...
  // todo [coral] [P10] strictly which convention is registered by planner should be hold in planner or place it in GlobalInfo
  //  && use weak reference for RelOptPlanner should be garbage collect
  public static Map<RelOptPlanner, Set<JdbcConvention>> conventionOfPlanner = new HashMap<>();
  ...
  public JdbcConvention(SqlDialect dialect, Expression expression, DataSource dataSource,
    ...
    GlobalInfo.getInstance().addConvention(this);
  }
  ...
  @Override public void register(RelOptPlanner planner) {
    ...
    // register converter rules only in Volcano Planner
      if (planner instanceof VolcanoPlanner){
        Set<JdbcConvention> jdbcConventionSet;
        if (!conventionOfPlanner.containsKey(planner)){
          jdbcConventionSet = new HashSet<>();
          conventionOfPlanner.put(planner, jdbcConventionSet);
        }else {
          jdbcConventionSet = conventionOfPlanner.get(planner);
        }
        if (!jdbcConventionSet.contains(this)){
          for (RelOptRule rule : JdbcRules.rules(jdbcConventionSet, this)) {
            planner.addRule(rule);
          }
          jdbcConventionSet.add(this);
        }
    }
  ...
  public boolean equals(Object o) {
    ...
    JdbcConvention that = (JdbcConvention) o;
    // todo [coral] [P10] is DataSource singleton ? or rewrite it's equals function && hash function
