...
import org.apache.calcite.plan.OptimizerKind;
...
//    private static ExecutorService executorService = Executors.newCachedThreadPool();
    ...
    @Getter
    private ThreadLocal<CalciteTimingTracer> timingTracerThreadLocal = new ThreadLocal<>();
    ...
    public OptimizerKind getFixedOptimizer() {
        CalciteConnectionConfig config = getConfigThreadLocal().get();
        if (config.fixedOptimizer() != null) {
            return OptimizerKind.valueOf(config.fixedOptimizer().toUpperCase(Locale.ROOT));
        }
        return null;
    }
