...
public class RelToSqlConverter extends SqlImplementor
  ...
  /** @see #dispatch */
  public Result visit(Join e) { // todo [coral] [P10] may flat to multi join so that clickhouse's syntax can compromise
   ...
   * todo [coral] [P10] JdbcToJdbcConverter -> ConverterImpl，同时将doMigration方法抽象到接口中
   */
  public Result visit(JdbcToJdbcConverter e){
    // do the migration job
    JdbcTable jdbcTable = e.doMigration();
    Result x = result(jdbcTable.tableName(),
            ImmutableList.of(Clause.FROM), e, null);
    final Builder builder =
            x.builder(e, Clause.SELECT);
    final List<SqlNode> selectList = new ArrayList<>();
    List<RelDataTypeField> fields = e.getInput().getRowType().getFieldList();
