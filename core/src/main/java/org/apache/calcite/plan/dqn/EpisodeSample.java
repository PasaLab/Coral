...
public class EpisodeSample implements IMDP, ITransformer {
    ...
    private Graph graph; // 查询图

    private CostModel costModel; //模型路径
    ...
    public void takeAction(Action action) {
        // 执行Join动作，将两个点合并成一个点，与两个点相关的边都连接到这个点上
...
//        graph.root.copy() // add nodes before first join
    ...
    public void mdp2DataFile() {
        ...
        final RelVisitor visitor = new RelVisitor() {
            ...
            public void visit(RelNode node, int ordinal, RelNode parent) { // 中序遍历
                if (node instanceof Join) {
                    joinStack.push((Join) node); // Join对应两个table，一个table pop一个
        ...
        for (Action action : actions) {
            INDArray input = Nd4j.concat(1, graph.toVector(), action.toVector()); // 将查询图和动作序列向量拼接起来
            RelMetadataQuery metadataQuery = graph.root.getCluster().getMetadataQuery();
            double cost = metadataQuery.getCumulativeCost(graph.root).getRows()  // 获取开销
